...
export function CreateProjectDialog({
  ...
  const handleAddDirectory = () => {
    const selected = window.prompt(t("dialog.remoteDirectoryPrompt"));
    const trimmedPath = selected?.trim();
    if (trimmedPath) {
      setPrompt((prev) => {
        if (hasEquivalentWorkingDir(prev, trimmedPath, null)) {
          return prev;
        }

        return insertWorkingDir(prev, trimmedPath);
      });
    }
  };
      ...
      <DialogContent className="max-w-lg max-h-[85vh] flex flex-col gap-0 p-0">
        ...
        >
          ...
          <ProjectIconPicker
            ...
            onChooseCustomIcon={chooseCustomIcon}
          />
