...
describe("ChatInput attachments", () => {
  ...
  it("attaches a browser file from the toolbar menu and sends it without text", async () => {
    const onSend = vi.fn();
  ...
  it("attaches a remote folder path from the toolbar menu", async () => {
    const user = userEvent.setup();
    vi.spyOn(window, "prompt").mockReturnValue("/Users/test/screenshots");
    mockInspectAttachmentPaths.mockResolvedValue([
      {
        name: "screenshots",
    ...
    await user.click(screen.getByRole("button", { name: /attach/i }));
    await user.click(screen.getByRole("menuitem", { name: /remote path/i }));

    expect(window.prompt).toHaveBeenCalledWith(
      "Enter a remote Goose server file or folder path",
    );
  ...
  it("renders mixed browser attachments from a single file picker pass", async () => {
    const user = userEvent.setup();
    ...
    await user.upload(input, [report, diagram]);
  ...
  it("dedupes path attachments that differ only by case on case-insensitive platforms", async () => {
    ...
    vi.spyOn(window, "prompt")
      .mockReturnValueOnce("/Users/test/report.pdf")
      .mockReturnValueOnce("/users/test/REPORT.pdf");
