...
const { mockOpenRemoteFileInChat } = vi.hoisted(() => ({
  mockOpenRemoteFileInChat: vi.fn().mockResolvedValue(undefined),
}));
...
vi.mock("@/shared/lib/browserPlatform", async () => {
  const actual = await vi.importActual("@/shared/lib/browserPlatform");
  return {
    ...(actual as typeof import("@/shared/lib/browserPlatform")),
    openRemoteFileInChat: mockOpenRemoteFileInChat,
  };
});
...
describe("MessageBubble", () => {
  beforeEach(() => {
    ...
    mockOpenRemoteFileInChat.mockClear();
  ...
  it("renders metadata attachments and opens them on click", async () => {
    ...
    expect(mockOpenRemoteFileInChat).toHaveBeenCalledWith(
